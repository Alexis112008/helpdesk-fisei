...
function DamageCatalogPage() {
  ...
  const [form, setForm] = useState({
    name: '', description: '', attentionLevel: 1
  });
    ...
    try {
      if (editItem) {
        await catalogAPI.put(`/damagecatalog/${editItem.id}`, {
          ...form, attentionLevel: parseInt(form.attentionLevel)
        });
      ...
      setShowForm(false);
      setEditItem(null);
      setForm({ name: '', description: '', attentionLevel: 1 });
      loadDamages();
    } catch {
  ...
  const getLevelName = (level) => {
    const names = { 1: 'N1 - Técnico Básico', 2: 'N2 - Técnico Profesional',
                    3: 'N3 - DITIC', 4: 'N4 - Proveedor' };
    ...
    <div style={styles.container}>
      <div style={styles.navbar}>
        ...
        <div style={styles.navRight}>
          ...
          <button onClick={() => { localStorage.clear(); navigate('/'); }}
            style={styles.logoutBtn}>Cerrar Sesión</button>
        </div>
      </div>
      ...
      <div style={styles.content}>
        <div style={styles.header}>
          <div>
            <h3 style={styles.title}>Catálogo de Daños</h3>
            <p style={styles.subtitle}>Categorías de incidencias y averías</p>
          </div>
          <button onClick={() => {
            setShowForm(true); setEditItem(null);
            setForm({ name: '', description: '', attentionLevel: 1 });
          }} style={styles.newBtn}>+ Nueva Categoría</button>
        </div>
        ...
        {loading ? (
          <p style={{ color: '#666', textAlign: 'center' }}>Cargando categorías...</p>
        ) : (
                ...
                {damages.map(d => (
                  <tr key={d.id} style={styles.tr}>
                    ...
                    <td style={styles.td}>
                      <span style={{
                        ...styles.badge,
                        backgroundColor: getLevelColor(d.attentionLevel)
                      }}>
                        {getLevelName(d.attentionLevel)}
                      </span>
                    </td>
                    <td style={styles.td}>
